...
public class Migrator {
    ...
    private static UpdateOneModel<Document> transformRating(Document doc) {
        try {
            String imdbRating = doc.get("imdb", Document.class)
                .getString("rating");
    ...
    public static void main(String[] args) {
        ...
        String mongoUri = "mongodb+srv://sv:sv@mflix.fen1u.mongodb.net/test";

        // instantiate database and collection objects
        MongoDatabase mflix = MongoClients.create(mongoUri)
            .getDatabase("sample_mflix");
        MongoCollection<Document> movies = mflix.getCollection("movies");
        Bson dateStringFilter = Filters.and(Filters.exists("lastupdated"),
            Filters.type("lastupdated", "string"));
        ...
        Bson ratingStringFilter = Filters.and(Filters.exists("imdb.rating"),
            Filters.type("imdb.rating", "string"));
